...
happyExpList = Happy_Data_Array.listArray (0,58) ([17616,32,512,61440,1,0,13312,26641,34,190,16384,33043,550,1101,2202,4404,128,17616,0,0,0,32768,3,7,4096,53279,68,124,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseHappy","Exp","let","in","int","var","'='","'+'","'-'","'*'","'^'","'/'","'('","')'","%eof"]
        bit_start = st * 17
        bit_end = (st + 1) * 17
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenPow p -> cont 13;
	TokenDiv p -> cont 14;
	TokenLParen p -> cont 15;
	TokenRParen p -> cont 16;
	_ -> happyError' ((tk:tks), [])
	}
...
parseHappy tks = happyRunIdentity happySomeParser where
